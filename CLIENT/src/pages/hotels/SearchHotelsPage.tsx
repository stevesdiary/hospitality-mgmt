...
const ALL_HOTELS = [
  { id: '1', name: 'Eko Hotel & Suites', city: 'Lagos', rating: 4.8, price: 52500, category: 'Luxury', grad: 'from-indigo-500 to-purple-700', amenities: ['Wifi', 'Pool', 'Gym'] },
  { id: '2', name: 'Transcorp Hilton', city: 'Abuja', rating: 4.9, price: 66500, category: 'Luxury', grad: 'from-cyan-500 to-blue-700', amenities: ['Wifi', 'Spa', 'Bar'] },
  { id: '3', name: 'Hotel Presidential', city: 'Port Harcourt', rating: 4.6, price: 38500, category: 'Standard', grad: 'from-emerald-500 to-teal-700', amenities: ['Wifi', 'Pool'] },
  { id: '4', name: 'Hamdala Hotel', city: 'Kano', rating: 4.4, price: 24500, category: 'Budget', grad: 'from-amber-500 to-orange-700', amenities: ['Wifi', 'Restaurant'] },
  { id: '5', name: 'Wheatbaker Hotel', city: 'Lagos', rating: 4.7, price: 48000, category: 'Boutique', grad: 'from-rose-500 to-pink-700', amenities: ['Wifi', 'Bar', 'Gym'] },
  { id: '6', name: 'Nicon Luxury', city: 'Abuja', rating: 4.5, price: 42000, category: 'Luxury', grad: 'from-violet-500 to-indigo-700', amenities: ['Wifi', 'Pool', 'Gym'] },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border transition-all ${
              showFilterPanel ? 'bg-primary-600 text-white border-primary-600' : 'bg-white text-gray-700 border-gray-200 hover:border-primary-400'
            }`}
          >
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['All', 'Luxury', 'Standard', 'Budget', 'Boutique'].map((c) => (
                      <button
                        key={c}
                        onClick={() => setSelectedCat(c)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                          selectedCat === c ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        }`}
                      >
                        {c}
                      </button>
                ...
                <div>
                  ...
                  <input type="range" min={10000} max={100000} step={5000} value={priceMax} onChange={(e) => setPriceMax(Number(e.target.value))} className="w-full accent-primary-600" />
                </div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {hotel.amenities.slice(0, 3).map((a) => (
                      <span key={a} className="inline-flex items-center gap-1 text-[10px] font-medium bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                        {a === 'Wifi' ? <Wifi className="h-2.5 w-2.5" /> : a === 'Gym' ? <Dumbbell className="h-2.5 w-2.5" /> : <Tag className="h-2.5 w-2.5" />}
                        {a}
                      </span>
                    ))}
                  </div>
        ...
        {results.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-24">
            ...
            <button onClick={() => { setLocation(''); setSelectedCat('All'); setPriceMax(100000); }} className="btn-outline text-sm">
              Clear search
            </button>
          </motion.div>
        )}
